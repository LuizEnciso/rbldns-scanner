...
void scan_connect(char **user, char *msg)
{
   ...
   {
      if(OPT_DEBUG)
         log_printf("SCAN -> %s is exempt from scanning", ipmask);
      return;
   }
      ...
      void *data)
{
   ...
   if(ss->manual_target == NULL)
   {
      ...
      irc_send_channels("PROXY ABIERTO -> %s %s:%d (%s) [%s]",
            ss->irc_nick, remote->ip, remote->port, scan_gettype(remote->protocol), scs->name);
      ...
      int notused, void *data)
{
   ...
   if (remote->bytes_read > 0) {
   	irc_send_channels("PROXY ABIERTO -> %s %s:%d (%s) [%s]",
           ss->irc_nick, remote->ip, remote->port, scan_gettype(remote->protocol), scs->name);

   	irc_send("%s GL * +%s 1296000 %ld :Acceso por proxy inseguro. Para informacion adicional, visite http://www.portalzona.org/proxyinfo.php", 
	   p10id, remote->ip, time(NULL));
	return;
   }
      ...
      void *data)
{
   ...
   if (remote->bytes_read > 0) {
        ...
        irc_send("%s GL * +%s 3600 %ld :Acceso por proxy inseguro. Para informacion adicional, visite http://www.portalzona.org/proxyinfo.php", 
           p10id, remote->ip, time(NULL));
        return;
   }
...
int scan_checkexempt(char *ipmask)
{
   node_t *node;
   char *exempt_mask;
...
int scan_checkcctld(const char *cctldUser)
{
   node_t *node;
   const char *cctld_permitido;

  LIST_FOREACH(node, CctldItem->cctld->head)
   {
      cctld_permitido = (char *) node->data;
      if (match(cctld_permitido, cctldUser)) {
          return 1;
      }
   }
