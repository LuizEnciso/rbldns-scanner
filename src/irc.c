...
#include <netdb.h> /* Necesario para usar gethostbyname */
#include <sys/types.h>
...
static void m_nick(char **, unsigned int, char *, struct UserInfo *);
static void m_eos(char **, unsigned int, char *, struct UserInfo *);
...
void irc_send_channels(char *data, ...)
{
   ...
   vsnprintf(data2, MSGLENMAX, data, arglist);
   va_end(arglist);
   snprintf(tosend, MSGLENMAX, "%sA%c P %s :%s", p10id, (p10nickcnt + 'A'), IRC_CHANNELS, data2);
   irc_send("%s", tosend);
}
...
static void irc_connect(void)
{
   ...
   inttobase64(p10id, IRCItem->numerico, 1);
   irc_send("PASS %s", IRCItem->password);

   irc_send("SERVER %s 1 %lu %ld J10 %s]]] +s6 :%s", IRCItem->name, time(NULL), time(NULL), p10id, IRCItem->desc);

   irc_send("%s N %s 1 %ld %s - %s ]]]]]] %sA%c :%s", p10id, IRCItem->nick, time(NULL), IRCItem->username, IRCItem->mode,
	p10id, (p10nickcnt + 'A'), IRCItem->realname);
   ...
   LIST_FOREACH(node, IRCItem->channels->head)
   {
      ...
      irc_send("%sA%c C %s %ld", p10id, (p10nickcnt + 'A'), channel->name, time(NULL));
      irc_send("%s M %s +o %sA%c", p10id, channel->name, p10id, (p10nickcnt + 'A'));
      irc_send("%s M %s +nts", p10id, channel->name);
   }
...
static void irc_parse(void)
{
   ...
   */
   ...
   for(i = 0; i < (sizeof(COMMAND_TABLE) / sizeof(struct CommandHash)); i++)
      if(strcasecmp(COMMAND_TABLE[i].command, parv[2]) == 0)
      {
...
static void m_ping(char **parv, unsigned int parc, char *msg, struct UserInfo *source_p)
{
   ...
   irc_send("%s Z %s %s", p10id, p10id, parv[3]);
}
...
static void m_nick(char **parv, unsigned int parc, char *msg, struct UserInfo *notused)
{
   char *user[4];
...
*/
   ...
   static char ipstring[HOSTIPLEN];
...
/*
   ...
   if (!isdigit(*parv[6]) || strlen(parv[6]) > 15) {
   	hostentry = gethostbyname(parv[6]);
...
*/
   ...
   if(scan_checkexempt(ipmask))
   {
      if(OPT_DEBUG)
         log_printf("GEOIP -> %s esta excluido de la examinacion", ipmask);
      return;
   }

   char *cctld = NULL;
   gidb = GeoIP_open(BOPM_ETCDIR"/GeoIP.dat", GEOIP_STANDARD);
   cctld = (char *)GeoIP_country_code_by_addr(gidb, user[3]);

   log_printf("GEOIP -> %s (%s) CCTLD: %s", user[0], user[3], cctld ? cctld : "Desconocido");

   if (cctld && scan_checkcctld(cctld)) {
	irc_send_channels("GEOIP -> %s (%s) esta conectando desde un pais prohibido (%s)", user[0], user[3], cctld);
	irc_send("%s GL * +%s 1296000 %ld :%s", p10id, ipstring, time(NULL), IRCItem->klinegeoip);
	return;
   }
   GeoIP_delete(gidb);
   scan_connect(user, "");
...
static void m_eos(char **parv, unsigned int parc, char *msg, struct UserInfo *notused)
{
   irc_send("%s EA", p10id);
}
