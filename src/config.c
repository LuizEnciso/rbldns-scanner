...
struct PaisesConf *CctldItem = NULL;
...
void config_init()
{
   ...
   /* Iniciar la lista de paises permitidos */
   CctldItem = MyMalloc(sizeof *CctldItem);
   CctldItem->cctld = list_create();
}
...
void config_setup()
{
   ...
   IRCItem->name = DupString("proxy.scanner");
   IRCItem->desc = DupString("RBLDNS Scanner");
   IRCItem->mode = DupString("+o");
   IRCItem->nick = DupString("Clones");
